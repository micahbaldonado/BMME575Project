...
cwd = os.getcwd()
soundFilePath = os.environ["HOMEPATH"] 
soundFilePath = os.path.join(soundFilePath, "OneDrive", "Gmail OneDrive", "Desktop", "sp01_babble_sn15.WAV")
output2 = os.environ["HOMEPATH"] 
output2 = os.path.join(output2, "OneDrive", "Gmail OneDrive", "Desktop", "output2.WAV")
...
logMMSE(soundFilePath, output2)

# read in properties/data from the noisy and denoised sound files
fs, noisy = read(soundFilePath)
fs, denoised = read(output2)
...
stois = []
# logmmse slightly trunkates the original shape of the noisy file. This is adjusted for with the following for-loop
for i in range(noisy_length-denoised_length):
    
    # pystoi code: https://github.com/mpariente/pystoi
    d = stoi(noisy[i:denoised_length+i], denoised, fs, extended=False)
    print(d)
    stois.append(d)

# I think the best solution to the trunkation is trunkating the end of the noisy file
print("I think this is the STOI:", stois[0])
